...
import com.share.commons.cache.config.CacheConfig;
...
import com.share.commons.util.AccessUtil;
...
public class CacheClient {
	...
	private Cache getCache(CacheSetting setting) {
		...
		try {
			...
			LogUtil.getApplicationLogger().error("error in buildCache", e);
			return null;
		}
	}
	...
	public CacheObject get(final String key) {
		...
		try {
			...
			LogUtil.getApplicationLogger().error("error in get", ex);
		}
		return obj;
	}
	...
	public void set(final String key, final Object value) {
		try {
			...
			LogUtil.getApplicationLogger().error("error in set", e);
		}
	}
	...
	private abstract class CallBack<T> {
		...
		protected void markErrorForGet(Cache cache, Exception e) {
			LogUtil.getApplicationLogger().error(getErrMsgForGet(cache), e);
		}
		...
		protected void markErrorForReSave(Cache cache, Exception e) {
			LogUtil.getApplicationLogger().error(getErrMsgForReSave(cache), e);
		}
	...
	private abstract class Save {
		...
		protected void markError(Cache cache, Exception e) {
			LogUtil.getApplicationLogger().error(getErrMsg(cache), e);
		}
	...
	private static class CostLogger {
		...
		void dolog() {
			if (sbd != null) {
				LogUtil.getApplicationLogger().info(sbd.toString());
			}
		}
	}
		...
		for (Cache cache : caches) {
			if (cache != null) {
				try {
					...
					LogUtil.getApplicationLogger().error("error in close", e);
				}
			}
		}
	}
