...
import com.share.commons.client.redis.config.RedisOption;
import com.share.commons.client.redis.extend.ExtendJedisCommands;
import com.share.commons.client.redis.provider.ClientProvider;
...
public class Parallel extends ClientProvider implements ExtendJedisCommands {
	...
	private static class Logger extends Checker {
		...
		<T> T check(T res) {
			if (access.access()) {
				if (res == null) {
					LogUtil.getApplicationLogger().info("parallel-null");
				} else {
					LogUtil.getApplicationLogger().info("parallel-notNull");
				}
			}
